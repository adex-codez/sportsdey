...
export const useFavorites = () => {
  ...
  const isFavoriteMatch = (id: string) => favoriteMatches.some(m => m.id === id);
  const isFavoriteLeague = (id: string) => favoriteLeagues.some(l => l.id === id);
  ...
  return {
    ...
    isFavoriteMatch,
    isFavoriteLeague,
    totalFavoritesCount: favoriteTeams.length + favoriteMatches.length + favoriteLeagues.length
  };
};
