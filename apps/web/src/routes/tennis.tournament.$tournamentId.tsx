...
export const Route = createFileRoute("/tennis/tournament/$tournamentId")({
...
function RouteComponent() {
	...
	const {
		...
		queryKey: ["tennis", "tournament", tournamentId, formattedDate],
		queryFn: () =>
			apiRequest<TennisScheduleData>(
				`tennis/tournament/${tournamentId}/schedule?date=${formattedDate}`,
			),
	});
			...
			<div className="mb-32 space-y-4 pb-10 lg:mb-0">
				<ErrorState
					message={
						isNetworkError
							? "Network Error"
							: "Failed to load tournament schedule"
					}
					description={
						isNetworkError
		...
		<div className="space-y-4">
			...
			{tennisLeagues.length === 0 ? (
				<EmptyState
					...
					description={
						activeFilter === "all"
							? "No tennis matches scheduled for this tournament"
							: activeFilter === "live"
