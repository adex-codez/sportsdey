...
cmsRoute.openapi(
	...
	async (c) => {
		try {
			...
			const query = `*[_type == "news" && !(_id in path("drafts.**")) ${search || type !== "all" ? `&& ${search ? "(title match $search || author->name match $search)" : ""} ${type && type !== "all" ? "&& category == $type" : ""}` : ""}] | order(${sortOrder}) [$start...$end] {
				_id,
				title,
				publishedAt,
				category,
			...
			const filteredQuery = `*[_type == "news" && !(_id in path("drafts.**")) ${search ? "&& (title match $search || author->name match $search)" : ""} ${type && type !== "all" ? "&& category == $type" : ""}] | order(${sortOrder}) [$start...$end] {
				_id,
				title,
				publishedAt,
				category,
...
cmsRoute.openapi(
	...
	async (c) => {
		try {
			...
			const query = `*[_type == "news"] | order(${sortOrder}) [$start...$end] {
				_id,
				title,
				publishedAt,
				category,
