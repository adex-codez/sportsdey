...
export const CurrentFilterProvider = ({ children }: { children: React.ReactNode }) => {
  const [currentFilter, setCurrentFilter] = useState<CurrentFilterType>("all");

  return (
    <CurrentFilterContext.Provider value={{currentFilter, changeCurrentFilter: setCurrentFilter}}>
      {children}
...
export const useCurrentFilter = () => {
  ...
  if (!currentFilterContext) {
    throw new Error("useCurrentFilter must be used within a CurrentFilterProvider");
  }
